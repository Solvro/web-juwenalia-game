  ...
  factory AppData.fromJson(
    ...
    return AppData(
      config: AppConfig(
        ...
        minAppVersionAndroid: (cfg['min_app_version_android']?.toString()) ?? '',
...
///
/// Priority: we fetch news, important_info, events, days first. That's
/// what the user sees when the shell lands — if those arrive on a weak
/// connection, the rest (partners, checkpoints, faqs) follows behind.
Future<AppData> fetchData(http.Client client) async {
  try {
    ...
    // the one we last cached, wipe per-edition progress before persisting.
  ...
  if (cached != null) {
    try {
      ...
      // corrupt cache — fall through to bundled asset
    }
  }
...
Future<AppData> _fetchFromDirectus() async {
  // Config first — everything else filters by edition.
  final config = await _fetchConfig();

  // Priority wave — what the first-visible tabs need.
  final primary = await Future.wait([
    _fetchNews(config.edition),
    _fetchImportantInfo(config.edition),
    _fetchSchedule(config.edition),
  ]);
  final news = primary[0] as List<NewsItem>;
  final importantInfo = primary[1] as List<ImportantInfo>;
  final schedule = primary[2] as List<ScheduleDay>;

  // Secondary wave — still needed, but behind the first paint.
  ...
  return AppData(
    config: config,
    checkpoints: secondary[0] as List<Checkpoint>,
    mapPoints: secondary[1] as List<MapPoint>,
    partners: secondary[2] as List<Partner>,
    faqs: secondary[3] as List<FaqItem>,
    partnerTiers: secondary[4] as List<PartnerTier>,
    news: news,
...
PlanBounds _parsePlanBounds(dynamic raw) {
  ...
  final e = numAt('east');
  final w = numAt('west');
  if (n == null || s == null || e == null || w == null) return PlanBounds.fallback;
...
Future<List<ScheduleDay>> _fetchSchedule(String edition) async {
  ...
  for (final e in raw.cast<Map<String, dynamic>>()) {
    ...
    final headlinerInstagram = artists.isNotEmpty
        ...
        : null;
    final headlinerSpotify = artists.isNotEmpty ? artists.first.spotifyUrl : null;
